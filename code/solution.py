...
class PCA():
    ...
    def _do_pca(self):
        ...
        '''
        n = np.ones((self.X.shape[1],1))
        X_comma = (1/self.X.shape[1])*self.X@n
        u,_,_ = np.linalg.svd(self.X - X_comma@n.T)
...
class AE(nn.Module):
    ...
    def _network(self):
        ...
        self.net = nn.Sequential(nn.Linear(self.n_features,640), nn.ReLU(), nn.Linear(640,self.d_hidden_rep),
            nn.Linear(self.d_hidden_rep,640), nn.ReLU(), nn.Linear(640,self.n_features)
        )
